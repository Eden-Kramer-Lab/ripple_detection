...
def simulate_LFP(
    ...
    *,
    ...
    ripple_amplitude : float, optional
        Peak amplitude of the ripple oscillation in the signal's units. Default
        is 2 when ``ripple_snr`` is not given. Cannot be combined with
        ``ripple_snr``.
    ...
    noise_type : {'white', 'pink', 'brown'}, optional
        Type of background noise. Default is 'brown'. Note that brown (1/f²)
        noise leaves very little power in the 150-250 Hz band -- about 1 % of
        the raw signal's standard deviation -- so ripples of any visible size
        dominate the band; 'pink' gives a ripple-band background closer to
        recordings. See Notes.
    noise_amplitude : float, optional
        Amplitude of background noise in the signal's units. Default is 1.3.
    random_state : int or np.random.RandomState, optional
        Seed or random state. The noise is drawn first, so a given seed
        produces the same noise whatever the ripple parameters. Default is
        None (nondeterministic).
    ripple_snr : float, optional
        Ripple size relative to the **ripple-band** background: the peak
        amplitude of each ripple after ripple-band filtering, divided by the
        standard deviation of the filtered noise. This is the quantity the
        detectors see. When given, ``ripple_amplitude`` is derived from it.
        Requires enough samples for ``filter_ripple_band`` (three times its
        kernel length). Default is None.
    ...
    ------
    ValueError
        If both ``ripple_amplitude`` and ``ripple_snr`` are given, or a range
        is not ordered.

    Notes
    -----
    ``ripple_snr`` is defined on the filtered signal, not on the envelope the
    detectors z-score. For a background whose envelope is roughly Rayleigh,
    the peak of a ripple in the z-scored consensus is about
    ``(ripple_snr - 1.25) / 0.65`` standard deviations, so ``ripple_snr=5``
    gives ripples near 6 SD and ``ripple_snr=3`` near 2.7 SD. With few, large
    ripples the ripples themselves inflate the SD and this mapping breaks
    down; keep ripples a small fraction of the record.
    ...
    if ripple_snr is not None:
        ...
        band_noise = filter_ripple_band(noise, sampling_frequency=sampling_frequency)
        # the burst is in band, so its filtered peak equals its raw peak
        ripple_amplitude = 2.0 * ripple_snr * band_noise.std()
    ...
    for ripple_time, frequency, duration in zip(
        ...
        carrier /= carrier.max()
        ripple_signal = np.sin(2 * np.pi * time * frequency)
