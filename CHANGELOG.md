...
- Four criteria the literature applies that the package could not express, taken from a survey of 57 decoding-replay papers (`docs/design/replay_detection_comparison/literature_detection_parameters.csv` in the companion analysis project):
  - `maximum_duration` on `Kay_ripple_detector`, `Karlsson_ripple_detector`, `Roumis_ripple_detector`, `Shvartsman_ripple_detector`, `Yu_ripple_detector`, `Carey_candidate_detector` and `multiunit_HSE_detector`; 25 of the 57 papers impose a ceiling, from 400 to 2000 ms, and only `Zugaro_ripple_detector` had one. The limit applies to the event as reported, after the bounds are extended past the threshold crossing, because that is what a published maximum describes; `minimum_duration` keeps its existing meaning of the run above threshold. Counted in samples through `sample_count_within`, so an event of exactly the limit is kept. A ceiling below the minimum raises. Default `None` detects exactly what each detector detected before.
  - `minimum_active_units` on `multiunit_HSE_detector`, with `n_active_units` now reported for every event. 21 of the 36 multiunit papers require a minimum, most often five; `Carey_candidate_detector` already applied the rule. Units are counted, not spikes. Default 0 imposes no criterion.
