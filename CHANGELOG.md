...
- `simulate_LFP` gains `ripple_snr` (ripple peak relative to the standard deviation of the ripple-band-filtered background, the quantity the detectors see; mutually exclusive with `ripple_amplitude`), `ripple_frequency` (scalar or a `(low, high)` range drawn per ripple; default 200 Hz as before), a `(low, high)` range option for `ripple_duration`, and `sampling_frequency` for the filtering `ripple_snr` needs (inferred from `time` when omitted). Existing calls produce byte-identical output: the noise is still drawn first, and the defaults are unchanged. The docstring no longer describes a ripple-to-noise amplitude ratio of 1.5 as realistic; with the default brown noise that ratio is about 135 times the ripple-band background.
